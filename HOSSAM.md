...
회귀분석의 목적, 왜 그런 현상이 발견했는가? 
각 독립변수가 내가 밝혀내고자하는 상황에 얼마나 지분이 있나 확인하는 것
...
- 실제 - 추정치
- 듀빈 왔쓴~ 
- r2 만 주고 adj.r2 랑 r 은 별도로 구해야함
- F 통계량 : 모형 자체에 대한 P VALUE
- 

- 머신러닝 상수항은 보통 자동으로 1 넣어주지만, 통계량을 구하려할 떄는 따로 상수항 처리 해줘야함
- 상수항이 없으면 절편 X > 원점을 지남



sklearn 은 상수항을 고려하지 않은 MSE 를 구하무르


- 각 독립변수
- p value 를 구하기 위한
- 각각 독립변수에 대해선s t값으로 구함
- 스케일링 여부와 상관없이 스케일링을 한 후에 B 계산
- apply : 내가 만든 함수를 모든 데이터 프레임에 적용해서 새로운 파생변수 만들 수있음



설명력 확인 > 과적합 확인 > 가정 조건 확인 (선형성 등) > 결과 보고
- order = 2 : 그래프에 곡선으로 표시 가능

[분석 프로세스]
수집 > 전처리 > 탐색적 데이터 분석 > 모형 적합 > 과적합 판정 > 가정확인 > 결과보고
